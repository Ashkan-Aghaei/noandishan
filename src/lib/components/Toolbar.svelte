  ...
  <div class="floating-panel !py-1 !px-3">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2">
        ...
        <div class="hidden lg:flex items-center space-x-2">
          <button
            ...
            title={$t('toolbar.prev_page')}
            aria-label={$t('toolbar.prev_page')}
          >
            <ChevronLeft size={30} />
          </button>
          ...
          <button
            ...
            aria-label={$t('toolbar.next_page')}
          >
        ...
        <div class="hidden lg:flex items-center space-x-2">
          <button
            ...
            aria-label={$t('toolbar.reset_zoom')}
          >
            <span class="font-medium text-xs">{$t('toolbar.reset')}</span>
          </button>

          <button
            class="tool-button h-8 w-auto flex items-center justify-center text-xs px-2"
            on:click={onFitToWidth}
            title={$t('toolbar.fit_width')}
            aria-label={$t('toolbar.fit_width')}
          >
            <span class="font-medium text-xs">{$t('toolbar.fit_width_short')}</span>
          </button>

          <button
            class="tool-button h-8 w-auto flex items-center justify-center text-xs px-2"
            on:click={onFitToHeight}
            title={$t('toolbar.fit_height')}
            aria-label={$t('toolbar.fit_height')}
          >
            <span class="font-medium text-xs">{$t('toolbar.fit_height_short')}</span>
          </button>
        </div>
      </div>
      ...
      <div class="hidden lg:flex items-center space-x-2">
        <button
          class="tool-button w-8 h-8 flex items-center justify-center"
          class:active={$drawingState.tool === 'hand'}
          on:click={() => handleToolChange('hand')}
        ...
        <div class="relative color-palette-container">
          <button
            ...
            aria-label={$t('toolbar.choose_color')}
          >
            <div
          ...
          {#if showColorPalette}
            <div class="absolute top-full mt-2 left-0 z-50">
              <div
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 min-w-[200px]"
              >
        ...
        <div class="relative line-width-container">
          <button
            ...
            title={$t('toolbar.brush_size')}
            aria-label={$t('toolbar.choose_line_thickness')}
          >
            <div
            ...
            >
              <div class="rounded-full bg-charcoal w-full h-full"></div>
            </div>
          </button>
              ...
              <div class="flex flex-col space-y-2 p-2">
                {#each availableLineWidths as width}
                  <button
                    ...
                    title={$t('toolbar.line_width') + " " + width + "px"}
                  >
                    <div
                      class="rounded-full bg-white dark:bg-white p-0.5 border border-gray-200 dark:border-white/30"
        ...
        {#if $drawingState.tool === 'eraser'}
          <div class="relative eraser-size-container">
            <button
              ...
              title={$t('toolbar.eraser_size')}
              aria-label={$t('toolbar.choose_eraser_size')}
            >
              <div
                class="rounded-full bg-white dark:bg-white p-0.5 border border-gray-200 dark:border-white/30"
                style="width: {Math.max($drawingState.eraserSize / 2 + 4, 10)}px; height: {Math.max(
                  $drawingState.eraserSize / 2 + 4,
                  10
                )}px;"
              >
                <div class="rounded-full bg-charcoal opacity-50 w-full h-full"></div>
              </div>
            </button>
                ...
                <div class="flex flex-col space-y-2 p-2">
                  {#each availableEraserSizes as size}
                    <button
                      ...
                      title={$t('toolbar.eraser_size_with') + " " + size + "px"}
                    >
                      <div
                        class="rounded-full bg-white dark:bg-white p-0.5 border border-gray-200 dark:border-white/30"
      ...
      <div class="flex items-center space-x-2">
        ...
        <div class="flex items-center space-x-2">
          ...
          >
            <Undo2 size={25} class="lg:w-3.5 lg:h-3.5" />
          </button>
          ...
          >
            <Redo2 size={25} class="lg:w-3.5 lg:h-3.5" />
          </button>
          ...
          <button
            ...
            title={$t('toolbar.delete_all')}
            aria-label={$t('toolbar.delete_all')}
          >
            <Trash2 size={25} class="lg:w-3.5 lg:h-3.5" />
          </button>
          ...
          <button
            ...
            aria-label={$t('toolbar.export_pdf')}
          >
            <Download size={25} class="lg:w-3.5 lg:h-3.5" />
          </button>
        </div>
        ...
        <div class="hidden lg:flex items-center space-x-2">
          ...
          <div class="flex items-center space-x-2 px-3 py-1.5 rounded-lg">
            <span class="text-sm text-charcoal dark:text-gray-200 capitalize"
          ...
          <button
            ...
            title={$t('toolbar.toggle_theme')}
            aria-label={$t('toolbar.toggle_theme')}
          >
            {#if $isDarkMode}
              <Sun size={25} />
            {:else}
        ...
        <div class="relative lg:hidden">
          ...
          >
            <MoreHorizontal size={25} class="lg:w-4 lg:h-4" />
          </button>
            ...
            <div class="absolute top-full mt-2 right-0 z-50 min-w[200px]">
              ...
              >
                ...
                <div class="mb-3 p-2 bg-sage/10 rounded-lg">
                  <div class="text-xs text-sage font-medium mb-1">{$t('toolbar.current_tool')}</div>
                  <div class="text-sm text-charcoal dark:text-gray-200 capitalize">
                    {$drawingState.tool}
                  </div>
                ...
                <div class="pt-2 border-t border-gray-200 dark:border-gray-600">
                  ...
                  >
                    <div class="flex items-center gap-2">
                      {#if $isDarkMode}
                        <Sun size={30} />
                        <span>{$t('toolbar.light_mode')}</span>
                      {:else}
                        <Moon size={30} />
                        <span>{$t('toolbar.dark_mode')}</span>
                      {/if}
                    </div>
                  </button>
                </div>
                ...
                <div class="pt-2 border-t border-gray-200 dark:border-gray-600">
                  <div class="flex items-center justify-between p-2">
                    ...
                    <a
                      ...
                      title={$t('toolbar.view_github')}
                      aria-label={$t('toolbar.view_github')}
                    >
                      <svg
                        width="14"
                        height="14"
...
<div class="toolbar-bottom fixed bottom-4 left-4 right-4 z-50 lg:hidden">
  <div class="floating-panel !py-2 !px-3 overflow-x-auto relative">
    ...
    <div
      ...
      class:opacity-0={!showLeftFade}
    ></div>

    <!-- Right fade indicator -->
    <div
      class="absolute right-0 top-0 bottom-0 w-4 bg-gradient-to-l from-white/80 dark:from-gray-800/80 to-transparent pointer-events-none z-[5]"
      class:opacity-0={!showRightFade}
    ></div>

    <div
      class="flex items-center space-x-2 min-w-max px-2"
      bind:this={toolbarScrollContainer}
      on:scroll={handleToolbarScroll}
    >
      <!-- Drawing tools -->
      <button
      ...
      <button
        ...
        title={$t('toolbar.sticky_note')}
        aria-label={$t('toolbar.sticky_note')}
      >
        <StickyNote size={25} />
      </button>
      ...
      <div class="relative stamp-palette-container">
        <button
          ...
          title={$t('toolbar.stamps')}
          aria-label={$t('toolbar.stamps')}
        >
          <Sticker size={25} />
        </button>
...
{#if showStampPalette}
  <div class="fixed bottom-20 left-4 z-[70] lg:hidden">
    <div
      class="bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-600/50 p-3 min-w-[280px]"
    >
      <div class="mb-2">
        <h3 class="text-sm font-medium text-charcoal/80 dark:text-gray-200 mb-3 text-center">
          {$t('toolbar.choose_stamp')}
        </h3>
        <div class="flex flex-wrap gap-3 justify-center">
          {#each availableStamps as stamp}
            <button
              class="sticker-preview group relative transition-all duration-300 hover:scale-110 hover:rotate-2 focus:outline-none focus:ring-2 focus:ring-sage/50 focus:ring-offset-2 {stamp.id ===
              $drawingState.stampId
                ? 'scale-110'
                : ''}"
